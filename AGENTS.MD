...
- GitHub quota: bare `gh` only (Octopool cache). Never bare `gh run watch` / `gh pr checks --watch` — 3s poll on real token. Watch = `gh run view <id>` at 30s/60s/120s backoff, or `gh run watch <id> -i 60`.
